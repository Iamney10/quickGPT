...
const SideBar = ({ isMenuOpen, setIsMenuOpen }) => {
    const { navigate, chats, setSelectedChats, theme, setTheme, user , createNewChat ,axios ,setChats ,fetchUsersChats,setToken ,token} = useAppContext()
    ...
    return (
        <div className={`flex flex-col h-screen min-w-72 p-5 bg-white dark:bg-[#000000] border-r border-[#80609F]/30 transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
            {/* logo */}
            ...
            <div className='mt-3 text-sm space-y-3'>
                {
                    chats.filter((chat) => chat.messages[0] ? chat.messages[0]?.content.toLowerCase().includes(search.toLowerCase()) : chat.name.toLowerCase().includes(search.toLowerCase())).map((chat) => (
                        <div onClick={() => { navigate('/'); setSelectedChats(chat); setIsMenuOpen(false) }} key={chat._id} className='p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-pointer flex justify-between group'>
                            <div>
